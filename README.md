...
- **gateway-proxy** — 将企业内网 AI 网关 V3 转换为 OpenAI 兼容接口，企业内部零成本，14+ 模型（推荐首选）
- **chrome-ext** — Chrome 浏览器扩展，增强 Dashboard 体验（拖拽文件插入路径、状态徽章等）
- **多模型方案** — 支持企业网关（推荐）、火山引擎（豆包）、Qwen/GLM/DeepSeek 等多种 LLM 接入方案
...
- **[gateway-proxy](gateway-proxy/)** — 将企业内网 AI 网关 V3 转换为 OpenAI 兼容接口，企业内部零成本，14+ 模型（推荐首选）
- **[chrome-ext](chrome-ext/)** — Chrome 浏览器扩展，增强 Dashboard 交互（拖拽文件插入路径、Native Messaging 路径解析）

## 文档目录

- [安装与迁移](docs/01-installation.md)
- [模型配置](docs/02-models.md)（企业 AI 网关为推荐首选，另含火山引擎豆包等方案）
